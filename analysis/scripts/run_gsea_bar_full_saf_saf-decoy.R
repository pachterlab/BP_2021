...
source("./gsea_bar_full_saf_saf-decoy.R")
...
walk2(dataset_meta$name_use, dataset_meta$species,
      function(x, y) {
        qq_df <- read_csv(paste0("../../data/DE_saf_saf-decoy/gsea_qq/", x, ".csv"))
        ...
        if (any(!is.na(qq_df$label))) {
          ...
          write_csv(qq_df, paste0("../../data/DE_saf_saf-decoy/gsea_qq/", x, ".csv"))
        }
      })
...
walk2(dataset_meta$name_use, dataset_meta$species,
      function(x, y) {
        bar_df <- read_csv(paste0("../../data/DE_saf_saf-decoy/gsea_bar/", x, ".csv"))
        go_def <- map_dfr(y, function(s) {
          read_csv(paste0("../../reference/GO/go_def", str_replace(s, " ", "_"), ".csv"))
        })
        bar_df <- bar_df %>% 
        ...
        write_csv(bar_df, paste0("../../data/DE_saf_saf-decoy/gsea_bar_name_fix/", x, ".csv"))
      })
